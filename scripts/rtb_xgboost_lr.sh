...
function init() {
    ...
    ###使用的训练模型
    use_model_type="xgboost_lr"                             # deepfm / xgboost /xgboost_lr / xdeepfm
...
function _xgboost_lr_train() {
    ...
    ${PYTHON3} ${APPS_DIR}/rtb/train/xgboost_lr_train_.py \
        ...
        --out_xgb_model_path "${TRAIN_DIR}/xgb_model.pkl" \
        --out_xgb_pmml_model_path "${TRAIN_DIR}/xgb_model.pmml" \
        --out_lr_model_path "${TRAIN_DIR}/lr_model.pkl" \
        --out_lr_pmml_model_path "${TRAIN_DIR}/lr_model.pmml" >> ${LOG_file}
...
function generate_test_data(){
    ...
    sed -i 's/^[01]#//' ${FEATURE_DIR}/test_encode.txt
