...
function init() {
    ...
    ###使用的训练模型
    use_model_type="xdeepfm"                             # deepfm / xgboost /xgboost_lr / xdeepfm
    ...
    LOG_file=${WORK_DIR}/logs/rtb/${ETL_DATE}/xdeepfm.log.${ETL_HOUR}
    touch ${LOG_file}
...
function _xdeepfm_predict() {
    ...
    ${PYTHON3} ${APPS_DIR}/rtb/predict/xdeepfm_predict.py \
        --log_path ${LOG_file} \
        --in_predict_input_file ${FEATURE_DIR}/test_encode.txt \
        --in_xdeepfm_model_path "${TRAIN_DIR}/xdeepfm_model.h5" \
        --out_predict_output_file ${PREDICT_DIR}/label_res.txt
