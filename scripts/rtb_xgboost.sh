...
#export PYTHON3=/apps/python/python3/bin/python3 #PROD
export PYTHON3=/usr/bin/python3 #FIBO PROD
#export PYTHON3=/usr/local/bin/python3 #HAN
...
function init() {
    ...
    ###使用的训练模型
    use_model_type="xgboost"                             # deepfm / xgboost /xgboost_lr / xdeepfm
    if_predict=true   # 是否要预估,跑批的时候不用 true / false

    DEL_DATE=$(date -d "${ETL_DATE} -5 days" "+%Y%m%d") # 删除过期数据的时间

    # 2个server服务
    SERVER_HOST=("vlionserver-ctr173" "vlionserver-ctr27")
    # SERVER_HOST=("vlionserver-ctr27")  #暂时先不更新杭州的

    # 相关文件名称
    PROJECT_NAME='rtb' # 如果有多个,这边需要切换(android,ios)

    # 目录相关 ################################
    DATA_DIR=/data/algorithm/data2/${PROJECT_NAME}/${use_model_type} #PROD
#    DATA_DIR=/Users/han/Documents/model/data/algorithm/data2/${PROJECT_NAME}/${use_model_type} #HAN
    ...
    LOG_file=${WORK_DIR}/logs/rtb/${ETL_DATE}/xgb.log.${ETL_HOUR}
    touch ${LOG_file}

    REMOTE_HTTP_SERVER_DATA_DIR=/home/apps/rtb_dsp_79_20210419 # # 远程接口服务器的数据路径
    #######################################################
...
function _preprocess_split_train() {
    ...
    ${PYTHON3} ${APPS_DIR}/rtb/preprocess/split_train.py \
        ...
        --out_file ${PREPROCESS_DIR}/train.txt  >> ${LOG_file}
...
function _feature_train() {
    ...
    ${PYTHON3} ${APPS_DIR}/rtb/feature/train_.py \
        ...
        --out_tne_file "${FEATURE_DIR}/tne.jpg"
...
function _xgboost_train() {
    echo "===输入文件总行数:" >> ${LOG_file}
    wc -l ${PREPROCESS_DIR}/train.txt >> ${LOG_file}
    ...
    ${PYTHON3} ${APPS_DIR}/rtb/train/xgboost_train.py \
        ...
        --out_vali_file "${FEATURE_DIR}/vali.txt" \
        --out_test_file "${FEATURE_DIR}/test.txt" \
        --out_feature_importance_file "${FEATURE_DIR}/feature_importance.jpg" \
        --out_xgb_model_path "${TRAIN_DIR}/xgb_model.pkl" \
        --out_xgb_pmml_model_path "${TRAIN_DIR}/xgb_model.pmml" >> ${LOG_file}
...
function _xgboost_lr_train() {
    ...
    ${PYTHON3} ${APPS_DIR}/rtb/train/xgboost_lr_train_.py \
        ...
        --out_xgb_model_path "${TRAIN_DIR}/xgb_model.pkl" \
        --out_lr_model_path "${TRAIN_DIR}/lr_model.pkl" >> ${LOG_file}
...
function generate_test_data(){
    cat  ${FEATURE_DIR}/vali.txt ${FEATURE_DIR}/test.txt > ${FEATURE_DIR}/test_encode.txt
    awk -F '#' '{print $1}'  ${FEATURE_DIR}/test_encode.txt > ${ETL_DIR}/test.label
    sed -i 's/^[01]#//' ${FEATURE_DIR}/test_encode.txt  #PROD CENTOS
#    sed -i "" 's/^[01]#//' ${FEATURE_DIR}/test_encode.txt  #HAN MAC

# 直接原始文件
#    head -500000 ${ETL_DIR}/train.txt >  ${ETL_DIR}/test.txt
#    awk -F '#' '{print $1}' ${ETL_DIR}/test.txt > ${ETL_DIR}/test.label
#    sed -i 's/^[01]#//' ${ETL_DIR}/test.txt
}
...
function _preprocess_split_predict() {
    ...
    ${PYTHON3} ${APPS_DIR}/rtb/preprocess/split_predict.py \
        ...
        --out_file ${PREPROCESS_DIR}/test.txt >> ${LOG_file}
...
function _auc() {
    ...
    ${PYTHON3} ${WORK_DIR}/src/apps/rtb/evaluate/auc.py \
        --log_path ${LOG_file} \
        --out_roc_curve_file ${PREDICT_DIR}/roc_curve.jpg \
        --out_ks_curve_file ${PREDICT_DIR}/ks_curve.jpg \
        --out_lift_curve_file ${PREDICT_DIR}/lift_curve.jpg \
        --out_gain_curve_file ${PREDICT_DIR}/gain_curve.jpg \
        --in_predict_real_file "${ETL_DIR}"/test.label \
        --in_predict_res_file ${PREDICT_DIR}/label_res.txt >> ${LOG_file} 2>&1
}

# 混淆矩阵
function _confusion_matrix() {
    # 校验(1,0) 准确度的
    env JAVA_OPTS="-Dfile.encoding=UTF-8 -Xms256m -Xmx512m" /opt/moudle/scala-2.12.8/bin/scala \
        -savecompiled ${WORK_DIR}/src/evaluate/confusion_matrix.scala \
        "${ETL_DIR}"/test.label \
        ${PREDICT_DIR}/label_res.txt  >> ${LOG_file} 2>&1
...
function update_online_models() {
    for server_host in ${SERVER_HOST[@]}; do
        ...
        scp ${FEATURE_DIR}/label_detail.json apps@${server_host}:${REMOTE_HTTP_SERVER_DATA_DIR}/${ETL_DATE}_${ETL_HOUR}/
